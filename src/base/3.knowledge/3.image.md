...
fn main() {
    ...
    fn search_classify(records: &mut Vec<ExamRecord>) {
        print!("\n请输入商品类别[0-6] :");
    ...
    fn search_goods(records: &mut Vec<ExamRecord>, mut goods: &mut String) {
        print!("\n请输入名称 :");
        io::stdout().flush().unwrap();
    ...
    fn payment(records: &mut Vec<ExamRecord>, money: f32) {
        ...
        if money > total_price {
            print_records(&rest_goods);
            println!("支付成功，剩余：{}", money - total_price)
        } else {
            println!("下单失败")
        }
    }
    ...
    loop {
        ...
        match choice.trim() {
            ...
            "2" => search_classify(&mut records),
            "3" => search_goods(&mut records, &mut goods),
